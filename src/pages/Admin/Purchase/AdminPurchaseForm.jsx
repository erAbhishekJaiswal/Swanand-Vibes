// // src/components/admin/AdminPurchaseForm.jsx
...
//  * AdminPurchaseForm
//  *
//  * Props: none required. Assumes you have an auth token interceptor for axios.
//  *
//  * Basic usage: render this in your Admin > Purchases > Create page.
//  *
//  * NOTE: Replace /api/products/search and /api/vendors endpoints as per your backend routes.
//  */
...
//     axios.get(`/api/products/search?q=${encodeURIComponent(searchQuery)}&limit=10`, { signal: controller.signal })
//       .then(res => setProductResults(res.data || res.data.products || []))
//       .catch(() => {})
...
//     // product from productResults: includes variants
//     updateLine(idx, {
...
//     // For quick demo: upload to server or to S3. Here we'll just keep File objects and send via multipart/form-data.
...
//       return;
//     }
//     const validItems = lines.filter(l => l.productId && Number(l.quantity) > 0);
...
//     // prepare body
//     // If uploading files: use FormData
...
//         invoiceFiles.forEach((f) => fd.append('invoiceFiles', f)); // backend should use multer.array('invoiceFiles')
//         const res = await axios.post('/api/purchases', fd, {
//           headers: { 'Content-Type': 'multipart/form-data' }
//         });
//         alert('Purchase created: ' + (res.data.purchase?.invoiceNumber || 'OK'));
//       } else {
//         const body = {
//           vendorId,
//           items: validItems.map(l => ({
...
//           paymentMethod,
//           notes,
//           invoiceFiles: [] // if you previously uploaded to S3, include URLs here
//         };
...
//         <div style={{ marginBottom: 12 }}>
//           <label>Search product (type to search)</label><br />
//           <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search product by name or SKU" />
//           {productResults.length > 0 && (
//             <div style={{ border: '1px solid #ccc', padding: 8, maxHeight: 200, overflowY: 'auto' }}>
//               {productResults.map(p => (
//                 <div key={p._id} style={{ padding: 6, cursor: 'pointer' }} onClick={() => onPickProduct(lines.length-1, p)}>
//                   <strong>{p.name}</strong> <br />
//                   <small>Brand: {p.brand} | Category: {p.category?.name || p.category}</small>
//                 </div>
//               ))}
//             </div>
//           )}
...
//           <h4>Items</h4>
//           {lines.map((ln, idx) => (
//             <div key={idx} style={{ border: '1px dashed #ddd', padding: 8, marginBottom: 8 }}>
//               <div style={{ display: 'flex', gap: 8 }}>
//                 <div style={{ flex: 2 }}>
//                   <label>Product</label><br />
//                   <input value={ln.productName} placeholder="Pick product (from search)" readOnly />
//                 </div>
//                 <div style={{ flex: 1 }}>
//                   <label>Variant / Size</label><br />
//                   <input value={ln.variantSize} onChange={e => updateLine(idx, { variantSize: e.target.value })} placeholder="M / L / color" />
//                 </div>
//                 <div style={{ width: 120 }}>
//                   <label>Qty</label><br />
//                   <input type="number" min="1" value={ln.quantity} onChange={e => updateLine(idx, { quantity: e.target.value })} />
//                 </div>
//                 <div style={{ width: 140 }}>
//                   <label>Purchase Price</label><br />
//                   <input type="number" step="0.01" value={ln.purchasePrice} onChange={e => updateLine(idx, { purchasePrice: e.target.value })} />
//                 </div>
//                 <div style={{ width: 120 }}>
//                   <label>Tax (amount)</label><br />
//                   <input type="number" value={ln.tax} onChange={e => updateLine(idx, { tax: e.target.value })} />
//                 </div>
//                 <div style={{ width: 80 }}>
//                   <label>Line Total</label><br />
//                   <div style={{ paddingTop: 6 }}>{formatCurrency((ln.quantity || 0) * (ln.purchasePrice || 0) + (ln.tax || 0))}</div>
//                 </div>
//                 <div style={{ width: 40 }}>
//                   <label>&nbsp;</label><br />
//                   <button type="button" onClick={() => removeLine(idx)} disabled={lines.length === 1}>✕</button>
//                 </div>
//               </div>
//             </div>
//           ))}
...
//           <label>Invoice files (optional)</label><br />
...
//           <div><strong>Total: ₹{formatCurrency(totals.grandTotal)}</strong></div>
//           <div>Total Qty: {totals.totalQty}</div>
//         </div>

//         <div style={{ marginTop: 12 }}>
//           <button type="submit" disabled={loading}>{loading ? 'Creating...' : 'Create Purchase'}</button>
//         </div>
//       </form>
//     </div>
//   );
...
export default function AdminPurchaseForm() {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `http://localhost:5000/api/products/common?search=${encodeURIComponent(
          searchQuery
        )}`,
